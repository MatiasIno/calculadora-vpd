...
const CACHE_NAME = 'vpd-calculator-v3'; // Cambia la versión aquí (v3, v4, etc.)
const urlsToCache = [
    './',
    './index.html',
    './style.css',
            ...
            return Promise.all(
                cacheNames.map((cacheName) => {
                    if (cacheName !== CACHE_NAME) {
                        return caches.delete(cacheName); // Borra cachés antiguas
                    }
                })
            );
        })
...
self.addEventListener('fetch', (event) => {
    event.respondWith(
        caches.match(event.request).then((response) => {
            return response || fetch(event.request); // Responde con caché o red
        })
    );
});
